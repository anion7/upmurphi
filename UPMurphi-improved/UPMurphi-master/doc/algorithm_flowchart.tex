...
%   Use fit to ensure algo boxes are fully inside
% ═══════════════════════════════════════════════════

% Phase 1 title (invisible anchor to position content)
\node[below=1.4cm of planner, font=\small, anchor=north] (p1title) {
  \phasetitle{phase1border}{Phase 1: 状态空间探索 (State Space Exploration)}
};

% Three algorithm boxes below the title
\node[algobox=algbfs,   below=0.5em of p1title.south, xshift=-7.5em] (bfs)   {BFS\\\scriptsize FIFO Queue};
\node[algobox=algdfs,   below=0.5em of p1title.south]                (dfs)   {DFS\\\scriptsize Stack};
\node[algobox=algastar, below=0.5em of p1title.south, xshift=7.5em]  (astar) {A*\\\scriptsize MinHeap\\[-2pt]\scriptsize $f\!=\!g\!+\!h$};

% Merge lines from three algo boxes
\coordinate (algmerge) at ($(dfs.south)+(0,-0.6em)$);
...
% Description text below the algo boxes
\node[below=1.2em of algmerge, anchor=north, text width=24em, align=left, font=\small]
  (p1body) {
...
% Draw the Phase 1 box around everything using fit
\begin{scope}[on background layer]
  \node[fit=(p1title)(bfs)(astar)(p1body),
        rectangle, rounded corners=5pt,
        draw=phase1border, line width=1.2pt, fill=phase1bg,
        inner xsep=12pt, inner ysep=10pt] (phase1) {};
\end{scope}
