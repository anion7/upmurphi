...
#define EXPLORE_ALG_ASTAR_FLAG "-vastar" /* verify with A* search. */
...
//UPMURPHI_END
{
  ...
  if (main_alg.mode == argmain_alg::Explore_bfs || main_alg.mode == argmain_alg::Explore_astar)
...
void argclass::ProcessOptions(string_iterator *options)
{
  ...
  for ( options->start();
    ...
    if( strcmp( option, EXPLORE_ALG_ASTAR_FLAG ) == 0 ) {
      main_alg.set(argmain_alg::Explore_astar);
      continue;
    }
...
//<< "\t-vdfs         depth-first search.\n"
       << "\t-vastar      breadth-first exploration using A* ordering.\n"
    ...
    case 1:
      ...
      switch( args->main_alg.mode ) {
        ...
        case argmain_alg::Explore_dfs:
          ...
          break;
        case argmain_alg::Explore_astar:
          cout << "A* best-first search.\n";
          break;
      ...
      if (  args->symmetry_reduction.value && (  args->main_alg.mode == argmain_alg::Explore_dfs ||
            args->main_alg.mode == argmain_alg::Explore_bfs ||
            args->main_alg.mode == argmain_alg::Explore_astar)) {
    ...
    case 1:
      if (args->print_progress.value && StateSet->NumElts() % args->progress_count.value == 0 )	{
        ...
        cout << "] "
             ...
             << "\n  ";
        if (args->main_alg.mode == argmain_alg::Explore_astar) {
          cout << "open states: " << StateSet->QueueNumElts() << ", ";
        } else {
          cout << "BFS level: "<< StateSet->CurrentLevel() << ", "
