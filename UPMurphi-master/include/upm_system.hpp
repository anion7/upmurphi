...
struct search_transition_t {
  unsigned long parent_index;
  RULE_INDEX_TYPE applied_rule;
  double step_cost;
};
...
class StateManager
{
  ...
  bool astar_mode;
  std::vector<double> astar_g;
  std::vector<double> astar_h;
  std::vector<double> astar_f;
  std::vector<unsigned long> astar_parent;
 ...
 public:
  ...
  bool Add(state * s, bool valid, bool permanent, unsigned long *index=NULL, bool *iserror=NULL,
           const search_transition_t* transition=NULL);
  ...
  inline bool InAStarMode() const
  {
    return astar_mode;
  }
  bool IsGoalState(unsigned long index) const;
  double GetAStarCost(unsigned long index) const;
  unsigned long GetAStarParent(unsigned long index) const;
  RULE_INDEX_TYPE GetAStarRule(unsigned long index) const;
