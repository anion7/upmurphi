...
/****************************************
  ...
  ****************************************/
state_queue::state_queue(unsigned long mas, bool use_priority, std::vector<double>* priority_ref)
  :  max_active_states(mas),
     priority_mode(use_priority),
     priority_counter(0),
     priority_reference(priority_ref)
{
  if (priority_mode) {
    stateArray = NULL;
    paging_file_top = NULL;
    paging_file_bottom = NULL;
    num_elts_head = num_elts_tail = 0;
    head_begin = tail_begin = head_size = tail_size = 0;
...
void state_queue::enqueue(state * &e, unsigned long index)
{
  if (priority_mode) {
    astar_entry entry;
    StateCopy(&(entry.node.s), e);
    entry.node.i = index;
    entry.tie_breaker = priority_counter++;
    if (priority_reference != NULL && index < priority_reference->size())
      entry.priority = (*priority_reference)[index];
    else
      entry.priority = 0.0;
    priority_queue_storage.push(entry);
#ifdef HASHC
    delete e;
#endif
    return;
...
{
  if (priority_mode) {
    Error.Notrace("Internal: Attempt to read an empty priority state queue.");
  }
#ifndef SPLITFILE
  size_t read =
