...
class state_queue
{
 protected:
  ...
  struct astar_entry {
    double priority;
    unsigned long tie_breaker;
    state_and_index node;
  };

  struct astar_compare {
    bool operator()(const astar_entry &lhs, const astar_entry &rhs) const
    {
  ...
  std::vector<double> *priority_reference;
  std::priority_queue<astar_entry, std::vector<astar_entry>, astar_compare> priority_queue_storage;
  state_and_index priority_last;
  state_and_index *priority_access(bool pop);

 public:
  // initializers
  state_queue(unsigned long mas, bool use_priority=false, std::vector<double>* priority_ref=NULL);
  ...
  virtual unsigned long NumElts(void)
  {
    if (priority_mode)
      return (unsigned long)priority_queue_storage.size();
    return num_elts_head + num_elts_tail + global_rear + global_front;
  }
  virtual bool isempty(void)
  {
    if (priority_mode)
      return priority_queue_storage.empty();
