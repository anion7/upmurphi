...
static const double ASTAR_G_EPS = 1e-9;

/************************************************************/
/* StateManager */
/************************************************************/
StateManager::StateManager(bool createqueue, unsigned long NumStates, bool use_priority)
  : NumStates(NumStates),num_goals(0),transitions(NULL),goals(NULL),errors(NULL),
    ...
    pno(1.0),num_errors(0),astar_mode(createqueue && use_priority)
{
...
bool StateManager::Add(state * s, bool valid, bool permanent, unsigned long *index, bool *iserror,
                       const search_transition_t* transition)
{
  unsigned long state_index;
  if (iserror!=NULL) (*iserror)=false;

  bool is_new = !the_states->was_present(s, valid, permanent, &state_index);

  if (!is_new) {
    if (astar_mode && transition!=NULL) {
      double parent_cost = 0.0;
      if (transition->parent_index < astar_g.size() &&
          astar_g[transition->parent_index] < std::numeric_limits<double>::infinity()) {
        parent_cost = astar_g[transition->parent_index];
      }
      double candidate = parent_cost + transition->step_cost;
      if (state_index < astar_g.size() &&
          candidate + ASTAR_G_EPS < astar_g[state_index]) {
        RecordAStarData(state_index,
  ...
  if (astar_mode && state_valid && state_index < astar_g.size()) {
    unsigned long parent_idx = (transition!=NULL) ? transition->parent_index : state_index;
    RULE_INDEX_TYPE parent_rule = (transition!=NULL) ? transition->applied_rule : 0;
    double parent_cost = 0.0;
    if (transition!=NULL &&
        transition->parent_index < astar_g.size() &&
        astar_g[transition->parent_index] < std::numeric_limits<double>::infinity()) {
      parent_cost = astar_g[transition->parent_index];
    }
...
{
  return astar_mode && index < astar_goal.size() && astar_goal[index]!=0;
}

double StateManager::GetAStarCost(unsigned long index) const
{
  if (!astar_mode || index >= astar_g.size())
    return std::numeric_limits<double>::infinity();
  return astar_g[index];
}
...
RULE_INDEX_TYPE StateManager::GetAStarRule(unsigned long index) const
{
  if (!astar_mode || index >= astar_rule.size())
    return 0;
  return astar_rule[index];
}
...
//BREAKPOINT("nextstate-gend")
  ...
  if (numstartstates>1) {
	...
	if ( answer=='y' || numstartstates<=1) {
		...
		if (StateSet->Add(nextstate, FALSE, TRUE, &state_index, NULL, NULL)) AddStartState(state_index);
...
#ifdef VARIABLE_WEIGHT
          if (weight>0) step_cost = (double)weight;
#elif defined(VARIABLE_DURATION)
          if (duration>0) step_cost = (double)duration;
#endif
          search_transition_t transition_info;
          transition_info.parent_index = from_index;
          transition_info.applied_rule = what_rule;
          transition_info.step_cost = step_cost;
          (void) StateSet->Add(nextstate, TRUE, permanent, &(next_indexes[nexts]),&iserror,&transition_info);
...
void AlgorithmManager::Plan()
{
  ...
  if (args->phase.value<=1) {
    if ( args->main_alg.mode == argmain_alg::Explore_bfs ) {
      ...
      Explore_dfs();
    } else if ( args->main_alg.mode == argmain_alg::Explore_astar ) {
      Explore_astar();
    }
  }
...
AlgorithmManager::Explore_bfs()
{
  ...
  StateSet = new StateManager(TRUE, NumStatesGivenBytes( args->mem.value ), false);
  MEMTRACKALLOC
  PAUSE
...
void
AlgorithmManager::Explore_astar()
{
  setofrules fire;
  bool deadlocked;
  state_and_index *sid;
  ...
  PO = new POManager;
  MEMTRACKALLOC
  StateSet = new StateManager(TRUE, NumStatesGivenBytes( args->mem.value ), true);
  MEMTRACKALLOC
  ...
  while ( !StateSet->QueueIsEmpty() ) {
    ...
    bool goal_expanded = StateSet->IsGoalState(curstateindex);
    if (goal_expanded) {
      expanded_goal = curstateindex;
      if (args->search_alg.mode == argsearch_alg::Feasible) {
        break;
      } else {
        continue;
      }
    ...
    if (enforce_horizon) {
      double gcost = StateSet->GetAStarCost(curstateindex);
      if (gcost > horizon_limit) {
        continue;
      }
    }
...
AlgorithmManager::Explore_dfs()
{
  ...
  PO = new POManager;
  MEMTRACKALLOC
  StateSet = new StateManager(FALSE, NumStatesGivenBytes( args->mem.value ), false);
  MEMTRACKALLOC
  ...
  while ((curstate = StartState->NextStartState()) != NULL) {
    (void) StateSet->Add(curstate, FALSE, TRUE, NULL, NULL, NULL);
    ...
    while ( !StateSet->QueueIsEmpty() ) {
      ...
      if ( nextstate!=NULL ) {
        if ( StateCmp(curstate,nextstate)!=0 ) {
          ...
          if (StateSet->Add(nextstate, TRUE, TRUE, NULL, NULL, NULL)) {
